...
function layoutChecks(
  ...
  return [
    ...
    // The DIRECTORIES a `test_paths` pattern walks from, not the pattern
    // text: `test/**/*.test.ts` is not a path on disk, and reporting it as
    // "missing" would send a correctly configured project looking for a
    // directory it never had. See `util/testPaths.ts`.
    ["test path", any(testScanDirectories(policy.testPaths))],
  ];
}
