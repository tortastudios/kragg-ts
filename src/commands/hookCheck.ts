...
import { buildReport, utcNow, EXIT_ENVIRONMENT } from "../engine/report.ts";
  ...
  try {
    ...
    const results =
      policy.baseline === undefined
        ? raw
        : applyBaseline(root, raw, readBaseline(root, policy.baseline), incremental ? scope.targets : undefined)
            .results;
