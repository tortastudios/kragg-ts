...
export function guardrailFiles(identity: ProjectIdentity): Record<string, string> {
  const manifest = packageJson(identity);
  return {
    "README.md": readme(identity),
    ".gitignore": GITIGNORE,
    ".node-version": `${NODE_VERSION}\n`,
    ".npmrc": NPMRC,
    "pnpm-workspace.yaml": pnpmWorkspace(identity.kind, identity.mcpSdk, selfPinned(manifest)),
...
/**
 * Dev dependencies, with `@tortastudios/kragg-ts` itself pinned only when this build IS a
 * released version.
 *
 ...
 *
 * The pin has a second consequence the generated `pnpm-workspace.yaml` has to
 * answer for: a released version is younger than the generated 30-day cooldown
 * for its first month, so `selfPinned` tells `pnpmWorkspace` to exempt exactly
 * this one scoped name whenever the pin is written. See `supplyChain.ts`.
 */
function devDependencies(): Record<string, string> {
  ...
  if (/^\d+\.\d+\.\d+/.test(version) && !version.startsWith("0.0.0")) {
    dependencies["@tortastudios/kragg-ts"] = version;
  }
  return dependencies;
}

/** Whether the generated manifest pins `@tortastudios/kragg-ts`, i.e. this build is a released version. */
function selfPinned(manifest: Readonly<Record<string, unknown>>): boolean {
  const dev = manifest["devDependencies"];
  return typeof dev === "object" && dev !== null && Object.hasOwn(dev, "@tortastudios/kragg-ts");
}
...
exemption is scoped to those package names (plus, in a project generated by a
released kragg, \`@tortastudios/kragg-ts\` itself, for the reason written beside it in that
file), nothing else, and the MCP entries should be deleted once the pinned
versions have aged past 30 days.
